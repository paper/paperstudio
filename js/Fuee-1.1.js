 ...
 * @version		1.1
 * 
 * [2010-8-25]	主要是修改命名的不规范
 * 
 * 	getElementPos  -> getPosition
	getElementPos.x -> getPosition.left
	getElementPos.y ->getPosition.top  
 */
(function(){
    var undefined, D=document,
	
	Fuee = window.Fuee = function(name){
    ...
    Fuee.fn.prototype = {
		...
		get:function(num){
			...
			if(elem.length){
				...
				throw Error('function get : elem is not array');
			}
		},
         ...
         * 支持标签与class的层级查找
         * 
         * Fuee("divId").find("p");							//即D.getElementById("divId").getElementsByTagName("p");
        ...
        hide: function(speed, callback){
            ...
            if (speed === undefined) {
                ...
                Fuee(elem).each(function(){
                    ...
                    var h = Fuee(self).getElemContentHeightWidth().height, w = Fuee(self).getElemContentHeightWidth().width, 
    ...
    Fuee.getBody = function(){
        return {
            height: D.body.clientHeight, //body对象高度		
            width: D.body.clientWidth //body对象宽度
        }
    };
    ...
    Fuee.getArea = function(){
        return {
            height: D.documentElement.clientHeight, //可见区域高度
            width: D.documentElement.clientWidth //可见区域宽度
        }
    };
    ...
    Fuee.getScroll = function(){
        return {
            top: Math.max(D.documentElement.scrollTop,D.body.scrollTop), //网页被卷去的高
            left: Math.max(D.documentElement.scrollLeft,D.body.scrollLeft) //网页被卷去的左
        }
    };
	...
	Fuee.drag.fn = Fuee.drag.prototype = function(oFuee){
		...
		elem = typeof oFuee.node == "string"?
				D.getElementById(oFuee.node):
				oFuee.node;

		if (oFuee.hand) {
			hand = typeof oFuee.hand == "string" ? 
					D.getElementById(oFuee.hand) : 
					oFuee.hand; 
		}
		else {
			hand = elem;
		...
		hand.onmousedown = function(e){
			...
			if (parentnode) {
				var px = Fuee(parentnode).getPosition().left, 
					py = Fuee(parentnode).getPosition().top,
