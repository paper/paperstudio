...
(function(){
    ...
    Fuee.fn.prototype = {
        ...
        getElem: function(num){
            return num === undefined ? this.elem : this.elem[num];
        },
       ...
       //the first child
        ...
        // FueeObject.attr("class");						//get elem's class name
        ...
        bestDisplay: function(){
            var elem = this.elem, tagname = elem.tagName, node = D.createElement(tagname);
        ...
        hide: function(speed, callback){
            ...
            if (speed === undefined) {
                ...
                Fuee(elem).each(function(){
                    ...
                    if (Fuee(self).css("display") == 'none') return; //如果本身就是隐藏的，就不执行
                        ...
                        (function(pos){
                            setTimeout(function(){
                                ...
                                if (pos == 0) {
                                    ...
                                    if (self.curDisplay != 'none') self.$oldDisplay = self.curDisplay;
            ...
            if (speed === undefined) {
                Fuee(elem).each(function(){
                    ...
                    if (self.$oldDisplay == undefined) {
                        ...
                        self.$oldDisplay == 'none' ? Fuee(self).bestDisplay() : self.style.display = self.$oldDisplay;
                    }
                });
            } else {
                if (typeof speed === "string") {
                ...
                Fuee(elem).each(function(){
                    ...
                    if (Fuee(self).css("display") == 'block' || Fuee(self).css("display") == 'inline') return; //如果本身就是显示的，就不执行
        ...
        sDown: function(speed, callback){
            ...
            Fuee(elem).each(function(){
                ...
                var self = this, h = Fuee(self).getElemContentHeightWidth().height, paddingTop = parseInt(Fuee(self).css("paddingTop")) || 0, paddingBottom = parseInt(Fuee(self).css("paddingBottom")) || 0;
                borderTop = parseInt(Fuee(self).css("borderTopWidth")) || 0, borderBottom = parseInt(Fuee(self).css("borderBottomWidth")) || 0, oldCSSTest = self.style.cssText; //以前的内联样式
                if (!Fuee.isIE()) //IE6 会卡~~
Fuee(self).setOpacity(0);
        ...
        sUp: function(speed, callback){
            ...
            Fuee(elem).each(function(){
                ...
                var self = this, h = Fuee(self).getElemContentHeightWidth().height, paddingTop = parseInt(Fuee(self).css("paddingTop")) || 0, paddingBottom = parseInt(Fuee(self).css("paddingBottom")) || 0, borderTop = parseInt(Fuee(self).css("borderTopWidth")) || 0, borderBottom = parseInt(Fuee(self).css("borderBottomWidth")) || 0, oldCSSTest = self.style.cssText; //以前的内联样式
                self.style.overflow = "hidden";
   ...
   //check firefox
    ...
    Fuee.extend({
                ...
                Fuee(node).bind({
                    "mouseover": function(){
                        ...
                        _this.timeOn = setTimeout(function(){
                            if (oFuee.on !== undefined) oFuee.on.call(_this);
                        }, oFuee.timeOn);
                    },
                    ...
                    "mouseout": function(){
                        ...
                        _this.timeOut = setTimeout(function(){
                            if (oFuee.out !== undefined) oFuee.out.call(_this);
                        }, oFuee.timeOut);
                    }
                });
            });
        ...
        changeColor: function(oFuee){
            var elem = this.elem, type = oFuee.type === undefined ? "backgroundColor" : oFuee.type, to = oFuee.to, from = oFuee.from === undefined ? false : oFuee.from, speed = oFuee.speed === undefined ? "normal" : oFuee.speed;
