...
(function(){
                        ...
                        (function(pos){
                            setTimeout(function(){
                                ...
                                if (pos == 100) {
                                    ...
                                    self.style.display = self.$oldDisplay || BJ(self).bestDisplay();;
                                    if (callback) 
                                        callback.call(self);
                                }
                            }, (pos + 1) * (speed + 1));
    ...
    BJ.isIE = function(v){
        ...
        if (+v == 6) {
            return msg.indexOf("MSIE 6.0") > -1 ? true : false;
        }
        if (+v == 7) {
	...
	BJ.drag.fn = BJ.drag.prototype = function(obj){
		...
		var oldOpacity = BJ(elem).css("opacity"),
			oldOpacity = oldOpacity <=1  ? oldOpacity * 100 : oldOpacity,
		...
		hand.onmousedown = function(event){
			...
			if (parentnode) {
				var px = BJ(parentnode).getElementPos().x, 
					...
					//父亲节点的border宽度也要剪掉，不要也会发上boder-width的偏移
			...
			elem.style.margin=0;							//清空margin，放在点元素发生瞬间位移
