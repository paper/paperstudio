...
(function($){
	 ...
	 *  t: current time（当前时间）；
		...
		obj={
			...
			endCallback:function(node){//...}
		}
	 */
	$.extend({
		"animateStart": function(obj){
			var node = this.elem, 
				speed = obj.speed || 80, 
			...
			var t = 0, b = parseInt(obj.from.left, 10), c = parseInt(obj.to.left, 10) - parseInt(obj.from.left, 10), d = speed, t2 = 0, b2 = parseInt(obj.from.top, 10), c2 = parseInt(obj.to.top, 10) - parseInt(obj.from.top, 10), d2 = speed;
			...
			node.tween = function(t, b, c, d, t2, b2, c2, d2){
				...
				if (t < d || t2 < d2) {
					...
					node.animateTime = setTimeout(function(){
						...
						//记录初始值
						node.b = parseInt(node.style[paraOne], 10);
						node.b2 = parseInt(node.style[paraTwo], 10);
						
						//记录变化量
						node.c = parseInt(obj.to.left, 10) - node.b;
						node.c2 = parseInt(obj.to.top, 10) - node.b2;
					...
					if(obj.moveCallback)
						obj.moveCallback(node);
				}
				else {
					//动画结束的时候，调用回调函数
					if (obj.endCallback) 
						obj.endCallback(node);
				}
			};
		...
		"animateContinue": function(callback){
			var node = this.elem;
