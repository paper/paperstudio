...
var telephoneBook=(function(){
	var fn=function(){
		...
		try{
			for(var i=0;i<=5;i++){
			if(!!Iphone.localStorage.getItem('tele'+i+'_number'))
				BJ('#tele'+i+'_number').innerHTML=Iphone.localStorage.getItem('tele'+i+'_number');
			
			if(!!Iphone.localStorage.getItem('tele'+i+'_name'))
				BJ('#tele'+i+'_name').innerHTML=Iphone.localStorage.getItem('tele'+i+'_name');
			}
		}catch(e){}
			...
			setTimeout(function(){
				...
				oh4.removeClass('loading');
					...
					Iphone.iphoneInnerHTML(html,function(f){
						...
						Iphone.createJS('iphone-1.0-telephone_book_edit.js');
						Iphone.moving('right');
					});
